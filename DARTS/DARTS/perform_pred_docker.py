...
from utils_docker import load_data, create_one_hot_seg, back_to_original_4_pred, back_to_original_4_prob, pickling
...
#         pred_oh[i] = create_one_hot_seg(out,segs)
    ...
    pred_orig = np.moveaxis(np.concatenate(pred_arg_max,axis = 0),0,-1)
...
#     pred_prob_orig = back_to_original_4_prob(pred_prob, orient, pad_sh1, sh1 , pad_sh2 , sh2, pad_sh3, sh3)
    ...
    nibabel.save(pred_orig_nib, os.path.join(args.segmentation_dir_path,'aparc.DKTatlas+aseg.deep.mgz'))
#   else:
...
#    if args.is_mgz:
    seg = nibabel.freesurfer.MGHImage.from_filename(os.path.join(args.segmentation_dir_path,'aparc.DKTatlas+aseg.deep.mgz'))
