...
from data_loader.load_neuroimaging_data_docker import OrigDataThickSlices
from data_loader.load_neuroimaging_data_docker import map_label2aparc_aseg
from data_loader.load_neuroimaging_data_docker import map_prediction_sagittal2full
from data_loader.load_neuroimaging_data_docker import get_largest_cc
from data_loader.load_neuroimaging_data_docker import load_and_conform_image
...
def fast_surfer_cnn(img_filename, save_as, logger, args):
    ...
    model.eval()
    prediction_probability_axial = torch.zeros((256, params_network["num_classes"], 256, 256), dtype=torch.float32)
    ...
    #temp1 = prediction_probability_axial.cpu().numpy() 
    #prediction_probability_axial_cpu = prediction_probability_axial.cpu().clone()
    #torch.cuda.synchronize() 
    #prediction_probability_axial = prediction_probability_axial.to(torch.device("cpu"))
    #del prediction_probability_axial    
    ...
    model.eval()
    prediction_probability_coronal = torch.zeros((256, params_network["num_classes"], 256, 256), dtype=torch.float32)
    ...
    model.eval()
    prediction_probability_sagittal = torch.zeros((256, params_network["num_classes"], 256, 256), dtype=torch.float32)
    ...
    intermediate_img = torch.mul(intermediate_img, 0.4)
    prediction_probability_sagittal = torch.mul(prediction_probability_sagittal, 0.2)
    intermediate_img = torch.add(intermediate_img, prediction_probability_sagittal)
    del prediction_probability_sagittal
    torch.cuda.empty_cache()
    torch.cuda.synchronize() 
    intermediate_img = intermediate_img.float()
    _, prediction_image = torch.max(intermediate_img , 3)
